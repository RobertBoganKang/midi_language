...
class MIDIChord(object):
    ...
    def __init__(self, numerator=4, ticks_per_beat=480):
        ...
        # calculate
        self.beats_to_capture = self.calculate_beats_to_capture()

    def calculate_beats_to_capture(self):
        beats_to_capture = []
        if self.numerator % 2 == 0:
            tmp = [(i + 1) * 2 for i in range(int(self.numerator / 2))]
            beats_to_capture.extend(tmp)
        elif self.numerator % 3 == 0:
            tmp = [(i + 1) * 3 for i in range(int(self.numerator / 3))]
            beats_to_capture.extend(tmp)
        else:
            # prime number
            prime_number = [1, 2, 3, 5, 7, 11]
            tmp = [x for x in prime_number if x <= self.numerator]
            beats_to_capture.extend(tmp)
        return beats_to_capture

    @staticmethod
    def note2pianoroll(notes, max_tick, ticks_per_beat):
    ...
    @staticmethod
    def greedy(candidates, max_tick):
        chords = []
        # start from 0
        start_tick = 0
    ...
    def extract(self, notes):
        ...
        candidates = {}
        # beats to capture
        for interval in self.beats_to_capture:
